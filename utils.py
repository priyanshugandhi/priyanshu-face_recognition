...
class ImageCoder(object):
    ...
    def decode_jpeg(self, image_data, look):
        if look == 'single':
            image = self._sess.run(self.image_standradisation, #self._decode_jpeg,
                                   feed_dict={self._decode_jpeg_data: image_data})
            images = []
            images.append(image)
            IMAGES_B = self._sess.run(self.image_batch, feed_dict={self.images: images})
            return IMAGES_B
...
def make_single_crop_batch(filename, coder):
